def compute_seed_mapping(input: str) -> list[list[int]]:
    ...
    for map_i in maps_splitted[1:]:
        ...
        for map_i_line in map_i_lines[1:]:
            dest_range_start, src_range_start, range_len = map(int, map_i_line.split(" "))
            
            for seed_mapping in mapping:
                if (seed_mapping[-2] >= src_range_start) and (seed_mapping[-2] < src_range_start + range_len):
                    seed_mapping[-1] = dest_range_start + (seed_mapping[-2] - src_range_start)
    return mapping
...
print(f"Closest seed is seed {closest_seed_mapping[0]} at distance {closest_seed_mapping[-1]}")
